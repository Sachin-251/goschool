  ...
  return (
    <div className="xl:w-1/3 md:w-1/2 w-full p-4 hover:scale-105 transition duration-700">
        <div className={`bg-gray-100 p-3 h-full shadow-lg shadow-black rounded-lg ${styles.card} w-full`}>
          {
            !image ? (
              <Image className="h-80 rounded w-full object-cover object-center mb-6" src={`/schoolImages/default.jpg`} alt="default" width={400} height={400} />
            ):(
              <Image className="h-80 rounded w-full object-cover object-center mb-6" src={`/schoolImages/${image}`} alt={`${name}`} width={400} height={400} />
            )
          }
